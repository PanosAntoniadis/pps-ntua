...
#define T 256
    for (t=0;t<T;t++) {
        ...
        if (north != MPI_PROC_NULL){
            MPI_Sendrecv(&u_current[1][1], local[1], MPI_DOUBLE, north, 0, &u_current[0][1],
                            local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &status );
        }

        // Communicate with south
        if (south != MPI_PROC_NULL){
            MPI_Sendrecv(&u_current[local[0]][1], local[1], MPI_DOUBLE, south, 0, &u_current[local[0]+1][1],
                            local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &status );
        }

        // Communicate with east
        if (east != MPI_PROC_NULL){
            MPI_Sendrecv(&u_current[1][local[1]], 1, column, east, 0, &u_current[1][local[1]+1],
                            1, column, east, 0, MPI_COMM_WORLD, &status );
        }

        // Communicate with west
        if (west != MPI_PROC_NULL){
            MPI_Sendrecv(&u_current[1][1], 1, column, west, 0, &u_current[1][0],
                            1, column, west, 0, MPI_COMM_WORLD, &status );
        }
